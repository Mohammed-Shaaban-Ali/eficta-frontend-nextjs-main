...
export const calculateReturnFlightFilterOptions = (matchingReturns: any[]) => {
  ...
  // Calculate airlines
  const airlinesMap = new Map<string, { id: string; text: string; count: number }>();
  
  // Calculate providers
  const providersMap = new Map<string, { id: string; text: string; count: number }>();
  
  // Calculate stops
  const stopsMap = new Map<number, { id: number; text: string; count: number }>();
  ...
  matchingReturns.forEach((flight: any) => {
    ...
    const stopText = legCount === 0 ? 'Direct' : legCount === 1 ? '1 Stop' : `${legCount} Stops`;
    ...
    if (price > 0) {
      actualMinPrice = Math.min(actualMinPrice, price);
      actualMaxPrice = Math.max(actualMaxPrice, price);
    }
  });

  // For return flights: min = 0, max = (actualMax - actualMin)
  const minPrice = 0;
  const maxPrice = actualMinPrice === Infinity || actualMaxPrice === 0 
    ? 5000 
    : Math.ceil(actualMaxPrice - actualMinPrice);

  const actualMin = actualMinPrice === Infinity ? 0 : Math.floor(actualMinPrice);
  const actualMax = actualMaxPrice === 0 ? 5000 : Math.ceil(actualMaxPrice);

  return {
    airline: Array.from(airlinesMap.values()).sort((a, b) => a.text.localeCompare(b.text)),
    stops: Array.from(stopsMap.values()).sort((a, b) => a.id - b.id),
    provider: Array.from(providersMap.values()).sort((a, b) => a.text.localeCompare(b.text)),
    minPrice,
    maxPrice,
