...
const useDropdowns = (dropdowns: Dropdown[]) => {
  const [values, setValues] = useState<DropdownValues>(() =>
    dropdowns.reduce((acc: DropdownValues, curr) => {
  ...
  const getHandlers = (title: string) => ({
    value: values[title],
...
const DropdownSelectBar = () => {
  ...
  const { dropdownComponents } = useDropdowns(dropdowns);
